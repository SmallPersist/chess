...
/*
 * 黑棋最开始是处于：等待红旗走棋的状态，刚启动的时候就在一直query
 * 红旗启动之后因为是先走，所以并没有什么请求，主要监听到合理的走棋就发送数据到server
 * 2个有没有可能会同时query？
 */
class BlackClient extends JFrame implements ActionListener, MouseListener{
	...
	public BlackClient(String title) throws Exception {
	...
	private void gameStart() throws Exception {
		...
		while(!_flag) {
			...
			if(!"no".equals(msg)) {
				_flag = true;
				update(msg);
				System.out.println("I am black, first query is ok:" + msg);
			}
